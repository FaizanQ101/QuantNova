    ...
    >
      ...
      >
        <img
          ...
          decoding="async"
        />
        <div className="absolute inset-0 bg-gradient-to-l from-[#070B14] via-[#070B14]/90 to-[#070B14]/70" />
      </div>

      {/* Content Container */}
      <div className="relative z-10 w-full h-full flex items-center px-4 sm:px-6 lg:px-[7vw]">
        {/* Image Card - Stacked on mobile, side-by-side on desktop */}
        <div
          ref={imageCardRef}
          className={`${
            isMobile
              ? 'relative w-full max-w-md mx-auto mb-8 rounded-2xl h-[300px]'
              : 'absolute left-[7vw] top-[22vh] w-[40vw] h-[56vh] max-w-[580px] max-h-[520px] rounded-[28px]'
          } overflow-hidden shadow-[0_24px_70px_rgba(0,0,0,0.45)] gpu-accelerate`}
          style={{ opacity: prefersReducedMotion ? 1 : isMobile ? 0 : 0 }}
        >
          <img
            src="/images/approach_card.jpg"
            alt="Our Approach"
        ...
        <div
          ...
          className={`${
            isMobile
              ? 'relative w-full text-center px-2'
              : 'absolute left-[54vw] top-[26vh] w-[38vw] max-w-[560px]'
          }`}
          style={{ opacity: prefersReducedMotion ? 1 : isMobile ? 0 : 0 }}
        >
          <span className="eyebrow block mb-3 sm:mb-4">APPROACH</span>
          <h2 className="text-[clamp(28px,6vw,52px)] sm:text-[clamp(32px,3.6vw,52px)] font-bold text-[#F4F6FF] leading-[1.0] mb-4 sm:mb-6">
            Systems that learn.
            <br />
            <span className="text-gradient">Design that lasts.</span>
          </h2>
          <p className="text-base sm:text-lg text-[#A7B1D8] leading-relaxed mb-6 sm:mb-8 max-w-[46ch] mx-auto lg:mx-0">
            We pair modern AI infrastructure with disciplined product craft—clear
            models, clean interfaces, and measurable outcomes.
          </p>
          <button className="inline-flex items-center gap-1 text-[#4F6DFF] hover:text-[#8B9FFF] transition-colors text-sm sm:text-base">
            Read our principles
